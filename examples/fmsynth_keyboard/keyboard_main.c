                           ...
                           FAR struct ap_buffer_s *apb)
{
  ...
  if (dump_enable)
    {
      int i;
      int16_t *samp = (int16_t *)apb->samp;
      for (i = 0; i < (apb->nbytes / 2); i++)
        {
          printf("%d\n", samp[i]);
        }
    }
...
static void enqueue_wavheader(FAR struct kbd_s *kbd,
                              FAR struct ap_buffer_s *apb)
{
  struct wav_header_s *hdr = (struct wav_header_s *)apb->samp;
  
  hdr->riff_chunk_id[0] = 'R';
  hdr->riff_chunk_id[1] = 'I';
  hdr->riff_chunk_id[2] = 'F';
  hdr->riff_chunk_id[3] = 'F';
  hdr->riff_chunk_size = 36 + APP_FS * APP_CHNUM * 2; /* Dummy */
  ...
  hdr->fmt_channel = APP_CHNUM;
  hdr->fmt_samples_per_sec = APP_FS;
  hdr->fmt_bytes_per_sec = APP_FS * APP_CHNUM * APP_BPS / 8;
  hdr->fmt_block_size = APP_CHNUM * APP_BPS / 8;
  hdr->fmt_bits_per_sample = APP_BPS;
  hdr->data_chunk_id[0] = 'd';
  hdr->data_chunk_id[1] = 'a';
  hdr->data_chunk_id[2] = 't';
  hdr->data_chunk_id[3] = 'a';
  hdr->data_chunk_size = APP_FS * APP_CHNUM * 2;  /* Dummy */

  apb->nbytes = sizeof(hdr);
...
int main(int argc, FAR char *argv[])
{
  ...
  fmsynthsnd_set_soundfreq(g_kbd.sound,
                           musical_scale[OCTAVE(4, MUSIC_SCALE_C)]);

  /* Render audio samples in audio buffers */

  dump_enable = false;
#if 0
  enqueue_wavheader(&g_kbd, g_kbd.nxaudio.abufs[0]);
  for (i = 1; i < g_kbd.nxaudio.abufnum; i++)
#else
