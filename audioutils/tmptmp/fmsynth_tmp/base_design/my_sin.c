...
#define SINRESOLUTION (USHRT_MAX + 1)
#define SINTABLE_DELTA ((M_PI / 2.) / 256.)
#define SINTABLE_REVDELTA (SINRESOLUTION / (M_PI / 2.))
...
float my_sin(float theta)
{
  ...
  quantization = quantization < 0 ? (SINRESOLUTION * 2) - quantization
                                  : quantization;

  rest = quantization & 0xFF;

  quantization %= (SINRESOLUTION * 4);

  phase   = quantization / SINRESOLUTION;
  quarter = quantization % SINRESOLUTION;

  if (phase & 0x01)
    {
      tblidx = 256 - (quarter >> 8) + 1;
      short_sin = s_sintbl[tblidx];
      short_sin = short_sin + (((s_sintbl[tblidx - 1] - short_sin) * rest) >> 8);
    }
  else
    {
      tblidx = quarter >> 8;
      short_sin = s_sintbl[tblidx + 1];
      short_sin = short_sin + (((s_sintbl[tblidx + 2] - short_sin) * rest) >> 8);
    }
  ...
  ret = (float)short_sin / (float)SHRT_MAX;

  result_sin = sinf(theta);
#if 1
  printf("theta=%f, phase=%d, rest=%d, quantaization=%d, quarter=%d, sin()=%f, ref_sin()=%f", theta, phase, rest, quantization, quarter,
      ret, result_sin);
  if (fabsf(ret - result_sin) >= 0.005)
    {
      float diff = fabsf(ret - result_sin);
      printf("  ERROR : %f\n", diff);
...
int main(void)
{
  ...
  for (theta = -(M_PI * 2.); theta < (M_PI * 4.); theta = theta + (SINTABLE_DELTA / 10.))
    {
      my_sin(theta);
