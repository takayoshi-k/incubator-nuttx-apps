/****************************************************************************
 * apps/audioutils/fmsynth/fmsynth_opfuncs.c
 *
...
#define PHASE_ADJUST(th) \
        ( ((th) < 0 ? (PSEUDOSIN_PI) - (th) : (th)) % (PSEUDOSIN_PI * 2) )
...
static opfunc_t opfuncs[] =
{
  [FMSYNTH_OPFUNC_SIN]      = pseudo_sin256,
  [FMSYNTH_OPFUNC_TRIANGLE] = triangle_wave,
  [FMSYNTH_OPFUNC_SAWTOOTH] = sawtooth_wave,
  [FMSYNTH_OPFUNC_SQUARE]   = square_wave,
};
...
static int pseudo_sin256(int theta)
{
  ...
  rest   = theta & 0x7F;
  phase  = theta / (PSEUDOSIN_PI / 2);
  tblidx = (theta % (PSEUDOSIN_PI / 2)) >> 7;
...
{
  theta = PHASE_ADJUST(theta);
  return theta < PSEUDOSIN_PI ? SHRT_MAX : -SHRT_MAX;
}
...
/****************************************************************************
 * name: fmsynth_get_opfunc
 ****************************************************************************/

FAR opfunc_t fmsynth_get_opfunc(unsigned int type)
{
  opfunc_t func = NULL;

  if (type < FMSYNTH_OPFUNC_NUM)
    {
      func = opfuncs[type];
    }
